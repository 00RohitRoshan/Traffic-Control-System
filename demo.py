...
if __name__ == '__main__':
    ...
    line = st.sidebar.number_input('Line position', min_value=0.0, max_value=1.0, value=0.6, step=0.05)
    st.sidebar.markdown('---')
    ...
    with car:
        # st.markdown('**Car**')
        car_text = st.markdown('  ')
    
    with bus:
        # st.markdown('**Bus**')
        bus_text = st.markdown('  ')

    with truck:
        # st.markdown('**Truck**')
        truck_text = st.markdown('  ')
    
    with motor:
        # st.markdown('**Motorcycle**')
        motor_text = st.markdown('  ')
    ...
    if track_button:
        ...
        if video_file_buffer :
            ...
            with torch.no_grad(): #torch.no_grad = doesn't produce gradient
                detect(opt, stframe, car_text, bus_text, truck_text, motor_text, line, fps_text,Text)
            ...
            st.sidebar.warning('Warning No Source selected ⚠️ 😡') 
