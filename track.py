...
def detect(opt, stframe, car, bus, truck, motor, line, fps_rate,Text):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_vid:
                ...
                # car.markdown(f"<h3> {str(len(data_car))} </h3>", unsafe_allow_html=True).empty()
...
def count_obj(box, w, h, id, label, line_pos):
    ...
    # if center_coordinates: print(center_coordinates)  # MyCode
